...
fn build_reader_context(
    ...
    Arc::new(ReaderContext {
        ...
        latest_commit_time: MAX_INSTANT_TIME.to_string(),
...
/// prefix on any mismatch. Thin wrapper over [`compare_against_gold`] (shared
/// with the core fg-reader harness) that adapts its `Result` to the
/// `#[test]`-per-function panic style used here.
fn assert_matches_gold(
    actual: &arrow_array::RecordBatch,
    gold: &arrow_array::RecordBatch,
    table_name: &str,
) {
    compare_against_gold(actual, gold).unwrap_or_else(|e| panic!("[{table_name}] {e}"));
}
...
fn test_read_record_batch_log_compaction() {
    ...
    let gold = read_gold_parquet(&gold_dir).expect("read gold parquet");
...
//
// The cpp `HoodieFileGroupReader` sets the schema handler on `ReaderContext`,
// and on the reader-v2 layout the core reader honors it end-to-end: the
// requested schema is applied as a projection, so the output carries exactly
// the requested columns. This test verifies both that projection (output is
// projected to `id`/`name` only) and merge correctness with the schema
// handler set.
// =============================================================================
...
fn test_read_record_batch_column_projection() {
    ...
    // On the reader-v2 layout, the requested schema on the ReaderContext's
    // schema handler IS honored end-to-end: the output is projected to exactly
    // the requested columns (`id`, `name`) — `age`/`ts`/meta columns are dropped.
    // (When ENG-42866 was first written against the older layout, projection
    // was not yet wired through the ReaderContext schema handler, so the output
    // carried all columns; reader-v2 now applies it correctly.)
    ...
    assert_eq!(
        out_cols,
        vec!["id", "name"],
        "output must be projected to exactly the requested columns"
    );

    // Merge correctness on the projected output: 2 sf rows, with the
    // log-applied update (Alice-V2) winning over the base (Alice).
    let ids = batch
        .column_by_name("id")
        .unwrap()
        .as_any()
    ...
    let names = batch
        ...
        .downcast_ref::<arrow_array::StringArray>()
        .unwrap();
    let mut records: Vec<(i32, String)> = (0..batch.num_rows())
