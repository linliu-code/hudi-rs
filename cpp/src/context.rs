...
///   repeated string completed_instants = 15;
///   repeated string inflight_instants = 16;
///   string archived_boundary = 17;
///   bool apply_completion_gate = 18;
/// }
/// ```
#[derive(Debug, Clone, Default)]
pub struct ReaderContext {
    ...
    /// C-INFLIGHT-DELTA completion-gate inputs (proto fields 15–18). Consumed
    /// only when `apply_completion_gate` is true (table version < 8 snapshot).
    pub completed_instants: Vec<String>,
    pub inflight_instants: Vec<String>,
    /// First active-timeline instant time; empty means None.
    pub archived_boundary: String,
    pub apply_completion_gate: bool,
}
...
/// Zips parallel key/value vectors from the FFI boundary into a [`HashMap`].
///
/// # Panics
///
/// Panics if `keys` and `values` differ in length. The proto contract
/// guarantees equal-length parallel arrays, so a mismatch is a bug in the
/// C++ caller. Truncating to the shorter vector (the default `zip` behavior)
/// would silently drop entries and mask that bug, so we fail loudly instead.
fn zip_kv(keys: Vec<String>, values: Vec<String>) -> HashMap<String, String> {
    assert_eq!(
        keys.len(),
        values.len(),
        "FFI key/value arrays are misaligned (keys vs values)"
    );
...
impl From<FfiReaderContext> for FileGroupReaderContext {
    fn from(ffi: FfiReaderContext) -> Self {
        ...
        let reader_context = ReaderContext {
            ...
            completed_instants: ffi.completed_instants,
            inflight_instants: ffi.inflight_instants,
            archived_boundary: ffi.archived_boundary,
            apply_completion_gate: ffi.apply_completion_gate,
        };
...
mod tests {
    ...
    fn test_empty_context_converts() {
        ...
        assert!(!ctx.reader_context.apply_completion_gate);
        assert!(ctx.reader_context.completed_instants.is_empty());
        assert!(ctx.reader_context.inflight_instants.is_empty());
        assert!(ctx.reader_context.archived_boundary.is_empty());
    }
    ...
    fn test_full_context_converts() {
        let ffi = FfiReaderContext {
            ...
            completed_instants: vec!["20240101120000000".into()],
            inflight_instants: vec!["20240101130000000".into()],
            archived_boundary: "20240101000000000".into(),
            apply_completion_gate: true,
        ...
        // C-INFLIGHT-DELTA completion-gate inputs carried through conversion.
        assert!(ctx.reader_context.apply_completion_gate);
        assert_eq!(
            ctx.reader_context.completed_instants,
            vec!["20240101120000000".to_string()]
        );
        assert_eq!(
            ctx.reader_context.inflight_instants,
            vec!["20240101130000000".to_string()]
        );
        assert_eq!(ctx.reader_context.archived_boundary, "20240101000000000");
        assert_eq!(
            ctx.reader_context
                .table_config
    ...
    #[test]
    fn test_zip_kv_aligned() {
        let map = zip_kv(vec!["a".into(), "b".into()], vec!["1".into(), "2".into()]);
        assert_eq!(map.len(), 2);
    ...
    #[test]
    #[should_panic(expected = "FFI key/value arrays are misaligned")]
    fn test_zip_kv_more_keys_than_values_panics() {
        zip_kv(vec!["a".into(), "b".into()], vec!["1".into()]);
    }

    #[test]
    #[should_panic(expected = "FFI key/value arrays are misaligned")]
    fn test_zip_kv_more_values_than_keys_panics() {
        zip_kv(vec!["a".into()], vec!["1".into(), "2".into()]);
    }
}
