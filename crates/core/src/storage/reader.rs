...
mod tests {
    ...
    async fn test_block_fetcher_reads_only_its_range() {
        ...
        // boundary. `read_content` takes `&self` and an explicit offset, so there is
        // no cursor for the earlier read to have moved — what the second call adds is
        // the boundary case, not independence.
